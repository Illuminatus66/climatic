# Climatic: Weather Data Downloader (Now with in-app Visualization)
...
## Features
**For Visitor**: Simply displays weather data in an HTML table for locations selected via a double-click event. No download or visualization options enabled.
...
**New Feature**: Visualize the weather data 
...
- [Tomorrow.io's](https://docs.tomorrow.io/reference/post-timelines) Weather API utilizing the free-tier services and data layers.
- [Nivo's](https://nivo.rocks/components/) 'Reactive' visualization library built on d3.js
...
2. Source the API keys and relevant connection URLs mentioned in the `.env.example` file to create your own `.env` file.
...
4. Change the `baseURL` in `src/api/index.js` file to reflect the domain name of your choice.
...
With these steps, you can easily deploy Climatic and start using it to download and visualize weather data.